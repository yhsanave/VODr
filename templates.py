...
def parse(template: str, args: TemplateArgs) -> str:
    return template.replace(TOURNAMENT_NAME, args.tournamentName).replace(TOURNAMENT_SHORT, args.tournamentShort).replace(TOURNAMENT_LINK, args.tournamentLink).replace(EVENT_NAME, args.eventName).replace(PHASE_NAME, args.phaseName).replace(ROUND_FULL, args.roundFull).replace(ROUND_SHORT, args.roundShort).replace(PLAYER_1, args.player1).replace(PLAYER_2, args.player2).replace(PLAYER_1_CHARS, args.player1Chars).replace(PLAYER_2_CHARS, args.player2Chars)